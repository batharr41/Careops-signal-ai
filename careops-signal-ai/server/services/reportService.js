...
export async function generateAgencyReport(agencyId, startDate, endDate) {
  ...
  // Fetch all alerts for the period
  const alertsResult = await pool.query(
  ...
  return new Promise((resolve, reject) => {
    ...
    const patientStats = patients.map(p => {
      ...
      return {
        ...
        criticals: pAlerts.filter(a => a.severity === 'critical').length,
        caregiver: p.caregiver_name || 'Not assigned',
        conditions: Array.isArray(p.medical_conditions) ? p.medical_conditions.join(', ') : ''
      };
    });

    // Sort by risk: critical first, then by highest score
